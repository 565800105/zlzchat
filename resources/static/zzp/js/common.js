/**
 * 通用方法封装处理
 * Copyright (c) 2019 smanor 57223165@qq.com
 */
var startLayDate;
var endLayDate;
...
var refreshItem = function(){
	...
	var target = $('.CNVP_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(dataId){
	...
	if ($.common.isNotEmpty(dataId)){
		...
		// 移除相应tab对应的内容区
		$('.mainContent .CNVP_iframe[data-id="' + dataId + '"]', topWindow).remove();
		return;
	}
	...
	if ($.common.isNotEmpty(panelUrl)){
		...
		$('.mainContent .CNVP_iframe', topWindow).each(function() {
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="CNVP_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" data-refresh="' + isRefresh + '" seamless></iframe>';
		if (isScrollToTop) {
			$('.mainContent', topWindow).find('iframe.CNVP_iframe').hide().parents('.mainContent').append(str1);
		} else {
			$('.mainContent', topWindow).find('iframe.CNVP_iframe').css({"visibility": "hidden", "position": "absolute"}).parents('.mainContent').append(str1);
		}
...
function activeWindow() {
	...
	return $('.CNVP_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
}
...
$.ajaxSetup({
    complete: function(XMLHttpRequest, textStatus) {
        if (textStatus == 'timeout') {
        	$.modal.alertWarning("服务器超时，请稍候再试！");
        	$.modal.enable();
            $.modal.closeLoading();
